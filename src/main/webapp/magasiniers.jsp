					...
					<div class="col-12">
						<div class="card">
							<div class="card-body">
								<h4 class="card-title">
									<caption>
										<c:if test="${magasinier != null}">
            			Editer
            		</c:if>
										<c:if test="${magasinier == null}">
            			Ajouter un nouveau magasinier
            		</c:if>
									</caption>
								</h4>
								<div class="card-body">
									...
									<div class="d-flex justify-content-center">
										<div class="mb-2" id='clickable'>
											<img
												src=" <c:out
															value="${magasinier == null ? 'assets/upload/images/13.jpg' : magasinier.image}" />"
									...
									<c:if test="${magasinier != null}">
										<input type="hidden" name="id"
											value="<c:out value='${magasinier.id}' />" />
									</c:if>

									<div class="row col-12">
										<fieldset class="form-group col-6">
											<i class="glyphicon glyphicon-user"></i> <label
												class="text-dark id="validationTooltip01"">Nom</label>
											<input type="text"
												value="<c:out value='${magasinier.nom}' />"
												class="form-control" id="validationTooltip01"
												name="reference" required>

											<div class="invalid-feedback">Le champ nom est
												requis.</div>
										</fieldset>
										<fieldset class="form-group col-6">
											<label class="text-dark">Prenom</label> <input
												type="text" value="<c:out value='${magasinier.prenom}' />"
												class="form-control" name="nom" required>
											<div class="invalid-feedback">Definissez un prenom s'il
												vous plait.</div>
										</fieldset>
									</div>
									...
									<div class="row  col-12">
										<fieldset class="form-group col-6">
											<label class="text-dark">Telephone</label> <input
												type="text" value="<c:out value='${magasinier.telephone}' />"
												class="input0 form-control" name="stock" required>
											<div class="invalid-feedback">Un numero est
												necessaire pour creer un magasinier.</div>
										</fieldset>

										<fieldset class="form-group col-6">
											<label class="text-dark">Address</label> <input
												type="text" value="<c:out value='${magasinier.address}' />"
												class="form-control" name="prix" required>
											<div class="invalid-feedback">L'address est
												requise.</div>
										</fieldset>
									</div>
									...
									<fieldset class="form-group p-1">
										...
										<input class="form-control" name="description" value="<c:out
												value='${magasinier.password}' />" />
										<div class="invalid-feedback">Le champ password est
											requis.</div>
									</fieldset>

									<c:if test="${magasinier != null}">
										<input type="text" name="path" value="/updateMagasinier"
											style="display: none;">
									</c:if>
									<c:if test="${magasinier == null}">
										<input type="text" name="path" value="/insertMagasiner"
											style="display: none;">
									</c:if>

									<input type="text" name="id_patron"
										value="<c:out value='${magasinier.getPatronId()}' />"
										style="display: none;">
