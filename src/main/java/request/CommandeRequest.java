...
public class CommandeRequest {
	...
	public void createCommande(String ref, String clientName, String produitName, String qte) {
		...
		int clientId = MagasinierDAO.getInstance().getMagId(clientName);
...
//					ProduitDAO.getInstance().UpdateQuantite(produitName, reste);
					if (Magasinier.getInstance() != null) {
						CaissierRequest.getInstance().UpdateMontant(Magasinier.getInstance().getNom(), total);
					}
				}
			}
	...
	public void DeleteCommande(String ref) {
		...
		try {
			...
			int confirm = JOptionPane.showConfirmDialog(null,
					...
					JOptionPane.INFORMATION_MESSAGE);
			if (confirm == JOptionPane.YES_OPTION) {
				CaissierRequest.getInstance().UpdateMontant(Magasinier.getInstance().getNom(), -GetPrice(ref));
				st.executeUpdate(sq);
				JOptionPane.showMessageDialog(null,
